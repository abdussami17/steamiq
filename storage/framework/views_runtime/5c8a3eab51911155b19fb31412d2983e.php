...
document.querySelectorAll('.card-unassign-btn').forEach(btn => {
    ...
    return `<span class="card-badges">${cards.map(c =>
        `<span class="card-chip ${map[c.type] || 'card-unknown'}"
            ...
            <button type="button"
                ...
                data-card-type="${esc(c.type || 'unknown')}">
                <i data-lucide="x"></i>
            </button>
        </span>`
    ).join('')}</span>`;
...
const ok = confirm(
    `Unassign this ${String(cardType || 'card').toUpperCase()} card?`
);
...
try {
    ...
    // reload scoreboard
    document.getElementById('pgSel').dispatchEvent(new Event('change'));
