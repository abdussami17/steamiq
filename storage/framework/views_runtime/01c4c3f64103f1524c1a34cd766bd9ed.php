...
<script>
    (function() {
        ...
        function buildTable(data, thead, tbody) {
            ...
            // Total fixed columns count: 5 meta + catNames.length + (bonus col if present) + (flags col if present) + 5 totals
            const totalCols = 5 + catNames.length + (hasBonusData ? 1 : 0) + (hasFlags ? 1 : 0) + 5;
            ...
            [{
                ...
                {
                    label: 'PLAYER POINTS',
                    cls: 'banner-total-player',
                    w: 100
                },
                {
                    label: 'GRAND TOTAL',
                    cls: 'banner-total-grand',
                    w: 100
                },
                {
                    label: 'RANK',
...
<script>
    (function() {
        ...
        async function exportLeaderboardExcelJS() {
            ...
            const META_COLS = ['Rank', 'No', 'Team Name', 'Member', 'Division'];
            ...
            const bannerRowValues = [...Array(META_COLS.length).fill('')];
            catNames.forEach((name, i) => bannerRowValues.push(catSlugs[i].toUpperCase()));
            if (hasBonusData) bannerRowValues.push('BONUS');
            bannerRowValues.push('TEAM POINTS', 'PLAYER POINTS', 'GRAND TOTAL', 'RANK', 'ORGANIZATION');
            ...
            const colLabelRowValues = ['RANK', 'NO', 'TEAM NAME', 'MEMBER', 'DIVISION', ...catNames];
            if (hasBonusData) colLabelRowValues.push('BONUS');
            colLabelRowValues.push('TEAM PTS', 'PLAYER PTS', 'GRAND TOTAL', 'RANK', 'ORGANIZATION');
            ...
            rows.forEach(row => {
                ...
                dataRowValues.push(isTeam ? row.player_points ?? 0 : row.total_points ?? 0);
                dataRowValues.push(isTeam ? row.grand_total ?? 0 : '');
    ...
    </script><?php /**PATH /home/u236413684/domains/voags.com/public_html/steamiq/resources/views/scores/scripts/score-script.blade.php ENDPATH**/ ?>