<script>
    (function () {
            ...
            fetch(url)
                ...
                .catch(() => {
                    document.getElementById('rosterTableBody').innerHTML =
                        '<tr><td colspan="9" class="text-center text-danger">Failed to load rosters.</td></tr>';
                });
        }
        ...
        function renderTable(rows) {
            ...
            if (!rows.length) {
                tbody.innerHTML = '<tr><td colspan="9" class="text-center text-muted py-4">No rosters found.</td></tr>';
                return;
            }
...
document.getElementById('exportGameCardBtn').disabled = checked.length === 0;
}

// Select all
document.addEventListener('change', function (e) {

if (e.target.id === 'selectAllRosters') {
...
if (e.target.classList.contains('roster-checkbox')) {
    ...
    const all = document.querySelectorAll('.roster-checkbox');
    const checked = document.querySelectorAll('.roster-checkbox:checked');

    const selectAll = document.getElementById('selectAllRosters');
...
const selected = [...document.querySelectorAll('.roster-checkbox:checked')]
    .map(cb => cb.value);

if (!selected.length) {
    alert('Please select at least one roster.');
    return;
}
...
// Selected roster IDs
selected.forEach(id => {
    url.searchParams.append('roster_ids[]', id);
});
...
.then(response => {
    ...
    if (!response.ok) {
        throw new Error('Export failed. Server returned ' + response.status);
    }

    const disposition = response.headers.get('Content-Disposition');

    let filename = 'game_cards.csv';
