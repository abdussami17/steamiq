...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function resetAll() {
        [sc_organizationDiv, sc_groupDiv, sc_subgroupDiv, sc_typeDiv, sc_studentDiv, sc_teamDiv, sc_activityDiv, sc_pointsDiv]
            .forEach(el => el.classList.add('d-none'));
        [sc_organizationSelect, sc_groupSelect, sc_subgroupSelect, sc_studentSelect, sc_teamSelect, sc_activitySelect]
    ...
    sc_eventSelect.addEventListener('change', function () {
        ...
        fetch(`/events/${currentEvent}/organizations`)
            ...
            .then(data => {
                ...
                data.forEach(o => sc_organizationSelect.innerHTML += `<option value="${o.id}">${o.name}</option>`);
            })
    ...
    sc_organizationSelect.addEventListener('change', function () {
        [sc_groupDiv, sc_subgroupDiv, sc_typeDiv, sc_studentDiv, sc_teamDiv, sc_activityDiv, sc_pointsDiv]
            .forEach(el => el.classList.add('d-none'));
        ...
        fetch(`/organizations/${this.value}/groups`)
            ...
            .then(data => {
                ...
                data.forEach(g => sc_groupSelect.innerHTML += `<option value="${g.id}">${g.group_name}</option>`);
            })
        ...
        fetch(`/groups/${this.value}/subgroups`)
            ...
            .then(data => {
                if (data.length > 0) {
                    ...
                    data.forEach(s => sc_subgroupSelect.innerHTML += `<option value="${s.id}">${s.name}</option>`);
                } else {
                    // No subgroups, show entity type selector directly
    ...
    sc_subgroupSelect.addEventListener('change', function () {
        [sc_typeDiv, sc_studentDiv, sc_teamDiv, sc_activityDiv, sc_pointsDiv]
            .forEach(el => el.classList.add('d-none'));
...
sc_entityType.addEventListener('change', function () {
    ...
    const params = new URLSearchParams({ group_id: groupId, sub_group_id: subGroupId });
    ...
    fetch(`/teams?${params.toString()}`)
        ...
        .then(data => {
            ...
            data.forEach(t => sc_teamSelect.innerHTML += `<option value="${t.id}">${t.name || t.team_name}</option>`);
        })
...
sc_teamSelect.addEventListener('change', function () {
    ...
    if (selectedEntityType === 'student') {
        ...
        fetch(`/team/${this.value}/students`)
            .then(r => r.json())
            .then(data => {
                sc_studentSelect.innerHTML = '<option value="">-- Select Player --</option>';
                data.forEach(s => sc_studentSelect.innerHTML += `<option value="${s.id}">${s.name}</option>`);
            })
        ...
        fetch(`/api/events/${currentEvent}/activities`)
            ...
            .then(data => {
                ...
                data.forEach(a => {
                    ...
                    let desc = ['brain'].includes((a.activity_type || '').toLowerCase()) ? a.brain_description || '' : a.egaming_description || '';
                    sc_activitySelect.innerHTML += `<option value="${a.id}">${desc ? name + ' - ' + desc : name}</option>`;
                });
            })
