...
if (this.value === 'student') {
    ...
    // Hide student until team selected
    sc_studentDiv.classList.add('d-none');
}
        ...
        fetch(`/api/events/${currentEvent}/activities`)
            ...
            .then(data => {
                ...
                data.forEach(a => {
                    ...
                    // 3️⃣ Append description based on activity_type
                    let description = '';
                    let type = (a.activity_type || '').toLowerCase();

                    if (type === 'brain' && a.brain_description) {
                        description = a.brain_description;
                    } else if (['playground','esports','egaming'].includes(type) && a.egaming_description) {
