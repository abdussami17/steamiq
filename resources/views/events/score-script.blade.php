...
<script>
    ...
    if (!eventId) {
        tbody.innerHTML = `<tr><td colspan="5">Please select an event</td></tr>`;
        return;
    }
    ...
    function renderScoreTable(tableData) {
    const tbody = document.getElementById('scoreBody');
    const thead = document.getElementById('scoreHead');

    if (!tableData || tableData.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5">No scores available for this event</td></tr>`;
        return;
    }
    ...
    tableData.forEach((row) => {
        ...
        bodyHtml += `
            <tr class="${row.type}-row">
                ...
                <td>${row.name}</td>
                <td>${row.activity}</td>
                <td class="total-cell"><strong>${row.total}</strong></td>
                <td>${row.rank}</td>
            </tr>
        `;
    });
